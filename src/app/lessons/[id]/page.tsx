      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-8 py-10">
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
            {prev ? (
              <Link href={`/lessons/${prev.id}`} className="text-sm text-text-dim hover:text-accent transition-colors">
                &larr; {prev.title}
              </Link>
            ) : <div />}
            {next ? (
              <Link href={`/lessons/${next.id}`} className="text-sm text-accent hover:text-text transition-colors">
                {next.title} &rarr;
              </Link>
            ) : <div />}
          </div>
